...
using PineConePro.Erp.MyModule.Infrastructure.Persistence;

namespace PineConePro.Erp.MyModule.Infrastructure.Persistence.Repositories;
...
internal class MyAggregateRepository : IMyAggregateRepository
{
    ...
    public async Task<IReadOnlyCollection<MyAggregate>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        // In a real implementation, you might use _context.MyAggregates.ToListAsync(cancellationToken);
        await Task.CompletedTask;
        return Array.Empty<MyAggregate>();
    }
